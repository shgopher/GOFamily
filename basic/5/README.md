...
这就是 从上往下的模式 其实跟从下往上一个意思 ，从下往上 就不用考虑 2i和2i+1了 只需要考虑 i/2 因为 取整数，
所以只需要一种即可。况且 它的 父节点只有一个啊，所以说从下往上比从上往下 判断的要多一个。

我们在插入的时候 可以直接 在最后插入，然后 插入的这个数据直接从下往上即可。插入的时候是按照层来插入的。

从上往下呢，需要判断你的堆顶到底是左边还是右边。
