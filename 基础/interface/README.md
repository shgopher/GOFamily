...
go语言使用一种叫做鸭子理论的方式去实现接口：只要实现了接口的方法（go泛型, go1.18+, 理论从方法改为了类型）就算是实现了这个接口，这属于隐式接口实现。即：接口和它的实现者之间为正交关系。
...
接口隔离原则（Interface Segregation Principle，ISP）：客户端不应该依赖于它不需要的接口。也就是说，一个类对另外一个类的依赖应该建立在最小的接口上。（**go推崇最小接口模式**）
