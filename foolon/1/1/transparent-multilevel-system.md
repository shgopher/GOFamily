...
在服务端做缓存就两个目的 1 缓解cpu的压力 2 缓解io的压力（比如数据库的io）

缓存需要考虑以下四个维度的问题：

1. 吞吐量 ： 缓存的性能体现
2. 命中率 ： 命中率越高价值越大否则性价比就没有了
3. 扩展功能 ： 比如要有时效时间，命中率的统计数据等功能
4. 分布式的考虑 

### 吞吐量
吞吐量必须在满足数据安全的情况，满足一定的命中率等各种限制情况下的吞吐量性能，否则没有任何意义，而且一般只在高并发状态下去考虑吞吐量，因为低负载的情况下考虑也没啥意义

进城内缓存为了避免读写的竞争问题，有两种解决方法 1 加锁，并且在读取或者写入的时候直接处理比如状态变更，统计，失效等功能，可以使用多段锁来进行优化 2 读取的时候将数据的写入和读取直接操作不加锁，然后对于操作步骤当作日志，然后对日志进行异步的操作。写入的时候引入一个队列来保证顺序。

### 命中率和淘汰的机制策略

一般的淘汰机制就三种

- fifo ：淘汰最早的 这种策略最差劲，不要用
- lru ： 淘汰最不活跃的那个最后一个即可，这个好点儿
- lfu ： 淘汰计数最低的那个最不常用的，看起来比lru好，但是加入了计时器，所以开销会变大，这个看取舍吧。
    - tiny lfu
    - w-tiny lfu

    这两种 lfu的变种更加复杂，命中率和淘汰机制更优秀

### 扩展功能

- 加载器 ： 可以主动去获取某些数据
- 淘汰策略 ： 可以自由的选择淘汰策略，比如是 用 w-tiny lfu 还是其它的算法
- 实效策略 ： 是某一个时间段后实效还是可以认为的让他实效
- 事件通知 ： 添加监听器
- 并发级别：基本上就是等于内部有几个分段 （这里说的都是进程中的缓存）
- 容量控制 ：指的是初始容量和最大容量
- 引用方式 ：这里是为了垃圾回收
- 统计信息 ： 统计命中率，平均加载，垃圾回收时间等 
- 持久化 ： 进程中的缓存没有意义，一般都是说的分布式的缓存

### 分布式缓存

- 复制式缓存 ： 适合经常读，但是不这么改变的数据，通常做法就是每个节点复制一份即可，就可以变成进程中的缓存那么快，但是更新的时候，就需要将数据变更到每一份数据
- 集中式缓存： 它是独立出来的跟任何的访问都不是一家，基本上可以无脑选redis作为目前阶段的缓存，因为基本上没有人用复制式的缓存，集中式尤其是redis已经成为既有标准了。而且redis还可以部署ap（高可用）的集群，非常的棒。当然如果用redis去作为分布式锁，通知，协调或者作为一个队列的话，还可以跟 例如zookeeper，etcd，Doozerd 等其它的分布式的中间件合作来实现更多的功能。


多级的缓存策略：

通常我们可以将访问进程内的缓存作为第一缓存，因为这个最快，然后如果没有就去redis集群这个第二级的缓存去查找数据，然后回填数据，返回数据，然后redis集群再没有就去数据库中寻找数据然后回填两次。优化方法是 读的时候优先读第一缓存的，如果没有再去实施刚才的策略，写的时候要优先写入到redis集群中，并且让第一缓存的实效，然后回填。

### 缓存的风险

- 缓存穿透 ： 查询的数据在数据库中压根不存在，那么缓存中也肯定不存在了，所以将从就把缓存给穿透了，解决方法是可以缓存这个空的没有的key，或者设置步隆过滤器

- 缓存击穿 ： 某些热点的请求，刚好在大负载请求的时候失效了，解决方法 1 添加分布式锁，如果发现缓存没有的话就不允许大量的请求请求一个数据，2 晚上没人用的时候更新

- 缓存雪崩：大批的数据实效，解决方法1，建立集群，变成高可用的系统，2，启用多级别的缓存机制 ，3 将实效时间进行随机化，避免同时实效。

- 缓存污染 ： 就是所谓的数据跟真实的数据不一致的情况，这跟ap模式不一样，因为高可用不代表数据练最终一致性都不要了，解决方法 1. 读数据先读缓存的，如果没有再去读数据源，然后回填，写数据直接写数据源，然后让缓存直接实效。其实回填之前也会出现数据不一致（就是当数据源更改了，然后删除缓存的数据之前那个时间点）但是ap就是会出现数据非常小的概率下的数据不一致，忍忍吧。
